...
-   **NORO Financial Model** (`/noro`): Password-gated financial dashboard (password: `NORO_PASSWORD` env or `"NoroAdmin!9"`) with two business unit views: "Shared Studios" (100% consolidated — all revenues, all costs) and "New Brand NORO" (NORO-only revenues: software + hardware, NORO payroll, NORO OpEx). Features 5 tabs (Assumptions, Revenue Model, P&L, Cash Summary, Annual Summary), year toggle 2026–2028, CSV export per tab, localStorage persistence. Calculation logic in `client/src/lib/noroModel.ts`. Assumptions include separate payroll and OpEx inputs for NORO vs Shared Studios, customer type config, hardware timing (upfront/delivery split), install lag, growth distribution by quarter for 2027–2028.
-   **Focus Score + Weekly Priorities Card**: Replaces all Phase 1/2/3 onboarding cards in the Entrepreneur Dashboard overview. Top half shows circular score ring with 3 category bars and an insight strip (color-coded: teal ≥70, gold 40–69, terracotta <40). Bottom half shows 5 AI-generated weekly priorities (GPT-4o-mini via `generateWeeklyPriorities` in `server/aiService.ts`); user selects 3 and confirms. Priorities regenerate each Monday. State stored in `ideas.data.weeklyPriorities` (`{ weekStart, priorities, selected, generatedAt }`). Endpoints: `GET/POST /api/weekly-priorities/:email` in both `server/routes.ts` and `backend/index.js`.
...
-   **Community-Free Membership Flow**: Users can sign up for a free membership after completing a Founder Focus Score quiz, requiring agreement to a contract. Dashboard logic adapts based on idea submission status. After Q8, a contact step asks for name + email; if provided, auto-creates account and sends a password setup email (no manual registration needed on results page). The "Tell Us What You Need" intake and "Smart Founder Snapshot" are merged into a single unified quiz (7 questions + optional message) that saves both `founderSnapshot` and `needsIntake` in one API call. On first login, a full-screen agreement gate requires entrepreneurs to accept the Entrepreneur Membership Agreement before accessing the dashboard, with an optional password change.
-   **AI Onboarding Flow (3 Phases)**: Phase 1: Coach-focused Welcome Card on first login ("Find your coach — it's free") with Snapshot as secondary CTA and 0% progress bar. Post-Snapshot shows stage badge, challenge, 90-day goal, and matched coaches with 100% progress bar. Phase 2: Engagement triggers (day-2 return, 2+ coach profile views) unlock Builder Mode prompt personalized with Snapshot idea. Dismissible, reappears next session. Phase 3: Builder Mode uses GPT-4o-mini to generate a draft business plan (Problem, Solution, Target Customer, Unique Value, 90-Day Goal, Key Risks) from Snapshot data. Upgrade nudge ($9.99/mo mentor) shown only after draft plan is generated. Existing "My Idea" and "My Business Plan" tabs remain unchanged. Engagement tracking uses localStorage (`tcp_coach_views`, `tcp_last_visit_date`, `tcp_builder_dismissed`, `tcp_builder_draft`).
