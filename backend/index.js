...
app.put("/api/entrepreneurs/intake/:email", async (req, res) => {
  try {
    ...
    if (resendData) {
      ...
      const needsList = needsIntake ? (needsIntake.needs || []).map(n => `<li style="margin-bottom:6px;">${n}</li>`).join("") : "";
      const focusStepsList = snapshotSummary ? (snapshotSummary.focusSteps || []).map((s, i) => `<li style="margin-bottom:6px;"><strong>Step ${i + 1}:</strong> ${s}</li>`).join("") : "";
      ...
      if (needsSection || snapshotSection) {
        try {
          await emailClient.emails.send({
            from: fromEmail,
            to: decodedEmail,
            subject: "Your Smart Founder Snapshot Summary - TouchConnectPro",
